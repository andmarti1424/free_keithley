...
class mclass:
    ...
    def __init__(self,  window):
        ...
        self.etr_load_impedance = Entry(window, textvariable=self.str_load_impedance, font='Helvetica 18', width=3)
        ...
        self.lbl_load_impedance.place(x = 350, y = 203)

        # min input signal voltage
        self.lbl_vin_min = Label(window, text="min input signal voltage", font='Helvetica 18')
        self.lbl_vin_min.place(x = 40, y = 263)
        self.lbl_vin_min = Label(window, text="Vrms", font='Helvetica 18')
        self.lbl_vin_min.place(x = 350, y = 263)
        self.str_vin_min = StringVar()
        self.str_vin_min.set(DEFAULT_VIN_MIN)
        self.etr_vin_min = Entry(window, textvariable=self.str_vin_min, font='Helvetica 18', width=3)
        self.etr_vin_min.place(x = 300, y = 260)
        self.etr_vin_min.icursor(1)

        # min input signal voltage
        self.lbl_vin_max = Label(window, text="max input signal voltage", font='Helvetica 18')
        self.lbl_vin_max.place(x = 40, y = 323)
        self.lbl_vin_max = Label(window, text="Vrms", font='Helvetica 18')
        self.lbl_vin_max.place(x = 350, y = 323)
        self.str_vin_max = StringVar()
        self.str_vin_max.set(DEFAULT_VIN_MAX)
        self.etr_vin_max = Entry(window, textvariable=self.str_vin_max, font='Helvetica 18', width=3)
        ...
        self.etr_points = Entry(window, textvariable=self.str_points, font='Helvetica 18', width=3)
        ...
        self.lbl_harm_qty.place(x = 40, y = 443)
        ...
        self.etr_maxy.place(x = 300, y = 500)
    ...
    def enable_siggen(self):
        ...
        if not SIM:
            ...
            self.send_cmd(':OUTP:AMPL 1') #;set amplitude in Vrms
    ...
    def clear(self):
        self.measurement = pd.DataFrame(columns = ['id', 'vin', 'vout', 'thd'])
        self.plots = 0 # number of plots done. can be up to 4
    #    self.plot()
    ...
    def change_state(self):
        ...
        if (self.but_start['text'] == "RUN"):
            ...
            step =  round((vin_max - vin_min) / (float(self.etr_points.get())-1), 2)
            #print(vin_min)
            #print(vin_max)
            #print(step, "\n----\n")
            ...
            for vin, row in sm.iterrows():
                ...
                self.str_details.set("Measuring THD at: " + format(sm['vin'][vin], ".2f") + " Vrms input")
                #if DEBUG: print("Measuring THD at: " , format(sm['vin'][vin], ".2f") , " Vrms input")
                #self.lbl_details.place(x = 40, y = 880)
...
#               else:
#                    self.set_siggen_freq(sm['vin'][vin])
